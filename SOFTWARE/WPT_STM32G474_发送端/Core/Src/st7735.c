...
//void ST7735_DrawFloat(uint16_t x, uint16_t y, float value, uint16_t color, uint16_t bgColor, const FontDef *font) 
//	{
//    char buffer[16]; // 用于存储格式化后的字符串
//    sprintf(buffer, "%.2f", value); // 将 float 转换为字符串，保留两位小数
//    ST7735_DrawString(x, y, buffer, color, bgColor, font); // 调用 ST7735_DrawString 打印字符串
//}
...
//void ST7735_PrintFloat(uint16_t x, uint16_t y, float value, uint16_t color, uint16_t bgColor, const FontDef *font) 
//	{
//    char buffer[16]; // 用于存储格式化后的字符串
//    sprintf(buffer, "%.2f", value); // 将 float 转换为字符串，保留两位小数，并添加 "V" 单位

//    uint16_t cursorX = x; // 当前绘制字符的 X 坐标
//    for (int i = 0; buffer[i] != '\0'; i++) {
//        ST7735_DrawChar(cursorX, y, buffer[i], color, bgColor, font);
//        cursorX += font->width; // 更新 X 坐标，确保字符不会重叠
//    }
//}
